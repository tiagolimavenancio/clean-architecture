  ...
  if (window) {
    ...
    return {
      ...
      getById(columnId: string) {
        const item = (this.getAll() as any[]).find((i: { id: string }) => i.id === columnId)!;
        return item as T;
      },

      removeById(columnId: string) {
        const idx = (this.getAll() as any[]).findIndex((i: { id: string }) => i.id === columnId)!;
      ...
      updateByField(columnId: string, field: any, newValue: any) {
        const item = (this.getAll() as any[]).find((i: { id: string }) => i.id === columnId) as T;
